...
public class ProxyServiceDelegate implements ArtifactDelegate {
    private static final Log log = LogFactory.getLog(ProxyServiceDelegate.class);
    ...
    public Artifacts getArtifactsList(String groupId, List<String> nodeList) {
        ...
        return databaseManager.fetchArtifacts(Constants.PROXY_SERVICES, groupId, nodeList);
    }

    @Override
    public Ack updateArtifact(String groupId, ArtifactUpdateRequest request) {
        log.debug("Updating proxy service " + request.getArtifactName() + " in node " + request.getNodeId()
                  + " in group " + groupId);
    ...
    private boolean updateProxyService(String groupId, ArtifactUpdateRequest request) {
