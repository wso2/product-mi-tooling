...
FOR %%c in ("%CARBON_HOME%\lib\*.jar") DO echo .\lib\%%~nc%%~xc | FINDSTR /C:"Saxon-HE" || (set CARBON_CLASSPATH=!CARBON_CLASSPATH!;".\lib\%%~nc%%~xc")
...
%_RUNJAVA% %JAVA_OPTS% -Dcarbon.home="%CARBON_HOME%" -Dcarbon.config.dir.path="%CARBON_HOME%\conf" -Ddefault.json.dir.path="%CARBON_HOME%\conf\config-parser" -Dorg.wso2.CipherTransformation="RSA/ECB/OAEPwithSHA1andMGF1Padding" -cp "%CARBON_CLASSPATH%" org.wso2.ciphertool.CipherTool %*
endlocal
:end
